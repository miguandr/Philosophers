...
/*   Updated: 2024/09/11 20:58:59 by miguandr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	assign_forks(t_philo *philos, t_mtx *forks, int num_philos)
{
	...
	while (i < num_philos)
	{
		...
		if (right_i < left_i)
		{
			philos[i].right_fork = &forks[right_i];
			philos[i].left_fork = &forks[left_i];
		}
		else
		{
			philos[i].right_fork = &forks[left_i];
			philos[i].left_fork = &forks[right_i];
		}
		i++;
	}
}
